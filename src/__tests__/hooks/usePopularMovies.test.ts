...
describe('usePopularMovies', () => {
  ...
  it('calls getPopularMovies with page', async () => {
    ...
    await waitFor(() =>
      expect(tmdbMocks.getPopularMovies).toHaveBeenCalledWith(3),
    );
  });

  it('refetches when page changes', async () => {
    const { rerender } = renderHook(({ p }) => usePopularMovies(p), {
      initialProps: { p: 1 },
    });
    await waitFor(() =>
      expect(tmdbMocks.getPopularMovies).toHaveBeenCalledWith(1),
    );
    rerender({ p: 2 });
    await waitFor(() =>
      expect(tmdbMocks.getPopularMovies).toHaveBeenCalledWith(2),
    );
  });
});
