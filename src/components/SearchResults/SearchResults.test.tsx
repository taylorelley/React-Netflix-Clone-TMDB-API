...
const renderResult = (
  props: Partial<ComponentProps<typeof SearchResults>> = {},
) =>
  render(
    <MemoryRouter>
...
describe('SearchResults', () => {
  ...
  it('calls setQuery with empty string on click', () => {
    ...
    fireEvent.click(
      container.querySelector('.search-results-item') as HTMLElement,
    );
  ...
  it('falls back to noImage on image error', () => {
    const { container } = renderResult();
    const img = container.querySelector('img.result-img') as HTMLImageElement;
    fireEvent.error(img);
